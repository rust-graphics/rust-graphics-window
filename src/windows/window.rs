...
impl Application {
    pub fn new(core_app: Arc<RwLock<dyn CoreAppTrait>>) -> Arc<RwLock<Self>> {
        //         Application {
        ...
        //             })),
        //         }
        let conf = vxresult!(core_app.read()).get_config();
        let application_name = string_to_cwstring(conf.get_application_name());
        ...
        if !conf.get_fullscreen() {
            window_rect.right = conf.get_window_width() as winapi::um::winnt::LONG;
            window_rect.bottom = conf.get_window_height() as winapi::um::winnt::LONG;
        }
        let dwex_style = if conf.get_fullscreen() {
            winapi::um::winuser::WS_EX_APPWINDOW
        } else {
            winapi::um::winuser::WS_EX_APPWINDOW | winapi::um::winuser::WS_EX_WINDOWEDGE
        };
        let dw_style = if conf.get_fullscreen() {
        ...
        if conf.get_fullscreen() {
            ...
            if screen_width != conf.get_window_width() as i32
                && screen_height != conf.get_window_height() as i32
            {
                let result = unsafe {
        ...
        let window = unsafe {
            winapi::um::winuser::CreateWindowExW(
                ...
                null_mut(),
                instance,
                transmute(Box::into_raw(Box::new(myself.clone()))),
            )
        };
        if window == null_mut() {
        ...
        {
            let mut myself = vxresult!(myself.write());
    ...
    pub fn set_renderer(&mut self, r: Arc<RwLock<RenderEngine>>) {
        vxlogi!("Render engine set.");
        self.renderer = Some(r);
    }

    pub fn run(&self) {
        while *vxresult!(self.running.lock()) {
    ...
    fn handle_message(
        ...
        match msg {
            winapi::um::winuser::WM_CLOSE => {
                ...
                *vxresult!(self.running.lock()) = false;
                // DestroyWindow(hWnd);
                // PostQuitMessage(0);
            }
